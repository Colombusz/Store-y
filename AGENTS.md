...
17. **Ring winding has two conventions — the render boundary must flip them.** The wire/DB format is RFC 7946 (`[lng, lat]`, CCW exterior, via `h3.cellsToMultiPolygon(cells, true)`). But three-globe's polygons layer feeds `geometry.coordinates` straight into `ConicPolygonGeometry`, which resolves the interior the **d3-geo spherical way — the opposite convention**. A ring left as-committed paints the **entire sphere** with the cap colour, leaving the real region as a hole (globe.gl issue #37; three-globe's own hexBin layer calls `.reverse() // correct polygon winding`). Fix: `toGlobeWinding()` in `frontend/src/features/regions/lib/geo-helpers.ts`, applied in `RegionPolygons` only — never change the stored geometry. Symptom of forgetting it: region commit "turns the whole world <colour>".
18. **Painting must lock the camera's rotate gesture.** globe.gl uses OrbitControls (`react-globe.gl` passes `controlType: 'orbit'`) whose canvas listeners still fire during a paint drag, so one gesture both paints and spins the globe. `useGlobeCamera.setRotationEnabled(false)` (`controls.enableRotate = false`) is driven by an effect in `RegionsPage` whenever a paint/erase tool is active; wheel-zoom and pan stay available for navigation. Note: `enableRotate` only exists on OrbitControls — if the control type ever changes to `trackball`, the equivalent flag is `noRotate` and the runtime check in `setRotationEnabled` must be updated.
