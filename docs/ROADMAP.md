...
**T-0.1 — Monorepo scaffold** · `DONE` (verified end to end: install → db:start → db:indexes ×2 → dev both servers → typecheck/test/build) · depends: —
**Goal:** `npm install && npm run db:start && npm run db:indexes && npm run dev` works from a clean clone.
**Deliverables:** root `package.json` (workspaces + scripts), `tsconfig.base.json`, `.gitignore` (`backend/data/`, `node_modules`, `dist`), `.env.example`, and manifests for `backend/`, `frontend/`, `packages/shared/` pinned to the versions in `TECH-DECISIONS.md`.
**Acceptance:** all three workspaces typecheck; `npm run dev` starts both servers; `npm test` runs (zero tests is fine).
**Notes:** npm workspaces only — pnpm/yarn are not installed. Done-notes for followers: `backend/scripts/db-indexes.ts` holds an intentionally empty `INDEX_MANIFEST` for T-0.3 to fill; `backend/src/server.ts` is a minimal bootstrap that T-0.4 replaces with `app.ts`/`env.ts`; the frontend `App.tsx` + inline styles are placeholders T-0.5 replaces; Vite is pinned to `host: '127.0.0.1'` (its default binds IPv6 `::1` only, which breaks IPv4 clients and the API proxy); companion deps (`tsx`, `@types/node`, `@types/react`, `@types/react-dom`) were added to the TECH-DECISIONS matrix; Phase 1+ libraries install with their owning tasks.

**T-0.2 — Shared contract package** · `DONE` (28 tests green across the 3 workspaces, including backend+frontend import smoke tests; verified no I/O/React imports) · depends: `T-0.1`
**Goal:** one place for schemas, types, constants and pure geo/date maths.
**Deliverables:** `packages/shared/src/{schemas,types,geo,dates,constants,index.ts}`; error-code constants; `spoiler.ts` exposing `applySpoilerFilter`.
**Acceptance:** imported cleanly by backend and frontend; contains no I/O and no React.
**Notes:** keep dependencies light — bundled into both sides. Done-notes for followers: `src/geo/` and `src/dates/` are deliberately not created yet — T-1.1 and T-3.1 own those files; `applySpoilerFilter` redacts any plain object with a valid `spoilerLevel` above the ceiling to `null` (root included — returns `null`), so services must filter lists with `filterBySpoilerLevel` first; `errorCodeSchema` currently knows only the generic codes — append domain codes as modules introduce them; `worldDateSchema` names the month field `month` (0-based) per DATA-MODEL §3, while ADR-0010's prose still says `monthIndex` — docs discrepancy to fix at next doc pass.
...
**Deliverables:** Vite config with an `/api` proxy, Tailwind 4 via `@tailwindcss/vite`, `main.tsx`, `App.tsx`, `routes.tsx`, `lib/api.ts` typed fetch client, TanStack Query provider, Zustand stores (`view`, `timeline`, `map`). The shell itself is built the component-first way: `App` composes `AppShell` (header/sidebar/outlet) from `components/layout/`, and no page component exceeds ~100 lines of composition.
