...
**T-1.5 — Hex painting interaction** · `DONE` (ray–sphere picking in `lib/ray-picking.ts` per §11.8; framework-free drag machine in `lib/paint/drag.ts` — RAF-coalesced (≤1 pick/frame) with per-cell change filtering; paint rules, live metrics and commit-payload builder in `lib/paint/{tools,metrics,commit}.ts`; `hooks/use-region-painting.ts` is a 35-line composition root over `use-paint-drag` + `use-paint-session`, with the globe ref **injected** by `useGlobeCamera` so no second, never-attached ref is created; `PaintPalette.tsx` presentational with `[`/`]` stepper, cell count, measured area, ±Δ, commit/cancel; single bulk commit via `POST /regions/bulk/cells`; RegionsPage blocks region selection while a paint tool is active and swaps inspector for palette; 87 frontend tests green across 14 files; typecheck and lint clean) · depends: `T-1.4`, `T-1.1`
...
**Notes:** cap embedded cells at 300 000 per region per ADR-0016; above the cap write the companion `regionCells` document in the same session.
**Notes:** the riskiest task in the project. Spike the picking first; keep hand-rolled spherical maths out of the diff (pitfall §11.8).
**Notes (T-1.5):** draft geometry renders through the existing `HexOverlay` working layer; the picking core is `lib/ray-picking.ts` (pure, no React) and the drag state machine `lib/paint/drag.ts` (framework-free, `createDragController`) — both unit-tested without a renderer. Acceptance coverage: area = Σ exact `cellArea` is asserted in `paint.test.ts` (world-radius scaled per §11.4), drag coalescing is asserted in `use-paint-drag.test.tsx` (moves coalesce to ≤1 pick per animation frame), and cancel is asserted in `use-region-painting.test.tsx` (no server call, draft cleared). End-to-end 50-cell/200-cell browser checks are deferred to T-1.6's manual pass. Follow-ups for T-1.6: `painting-on-other-region` is accepted by the client (server rejects); the 300 000-cell cap and `regionCells` overflow are unimplemented. **Fixed post-T-1.5:** committed-region polygons rendered with inverted winding (whole globe painted the cap colour — AGENTS.md §11.17; fix `toGlobeWinding` in `geo-helpers.ts`, applied in `RegionPolygons`) and painting did not lock camera rotation (§11.18; fix `useGlobeCamera.setRotationEnabled` + effect in `RegionsPage`). 96 frontend tests green. **Erase redone (Photoshop model, live-verified with headless Chrome screenshots):** no target, no seeding — the draft holds REMOVED cells, each pick resolves lat/lng at every region's own resolution (`lib/paint/erase.ts`: `buildEraseIndex`/`eraseCellsAt`/`computeEraseCommits`/`makeEraseMetrics` + `computeEraseOverrides`), commit fans out to one bulk commit per touched region, and the palette hides the meaningless Res stepper in erase. Regions keep their smooth polygon shape while erasing: touched regions show the union of their remaining cells via `geometryOverrides` (fully-erased hide) instead of hex tiling. **Data repair:** legacy regions with cells but `geometry: null` were invisible with fictional areas (the seed itself shipped 7: `npm run repair:regions` backfills geometry/area/bbox/centroid idempotently, tested); the seed now derives all region geometry from cells. Split actions extracted to `use-split-actions` to hold the `RegionsPage` size budget. 97 backend + 131 frontend tests green.
